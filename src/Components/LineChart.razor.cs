...
public partial class LineChart
{
    ...
    [Parameter]
    public Action<LineChartOptions>? Options { get; set; }
    public LineChartOptions? _options { get; set; } = new();
    private LineChartSettings? _settings { get; set; }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            ...
            await JSRuntime.InvokeVoidAsync("gcLineChart", DataTable is not null ? DataTable!.Build() : ObjectArray, _settings, id, objectReference, DataTable is not null ? true : false, UseMaterialTheme);
            StateHasChanged();
        }
    }
