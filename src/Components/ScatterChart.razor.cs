...
public partial class ScatterChart
{
    ...
    [Parameter]
    public Action<ScatterChartOptions>? Options { get; set; }
    public ScatterChartOptions? _options { get; set; } = new();
    private ScatterChartSettings? _settings { get; set; }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            ...
            await JSRuntime.InvokeVoidAsync("gcScatterChart", DataTable is not null ? DataTable!.Build() : ObjectArray, _settings, id, objectReference, DataTable is not null ? true : false, UseMaterialTheme);
            StateHasChanged();
        }
    }
